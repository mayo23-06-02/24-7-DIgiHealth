...
export default function BookConsultCard({
  isOnline,
  pushToQueue,
}: {
  isOnline: boolean;
  ...
  return (
    <div className="bg-white rounded-lg border border-slate-100 shadow-[0_8px_30px_rgb(0,0,0,0.04)] p-8">
      ...
      <p className="text-slate-600 font-medium mb-8">
        Connect with an HPCSA registered doctor immediately via secure video
        call.
      </p>
      ...
      {bookingStatus === "success" ? (
        <div className="bg-green-50 border border-green-200 text-green-700 p-6 rounded-2xl flex flex-col items-center justify-center gap-3 animate-reveal">
          ...
          <p className="font-bold text-center">
            {isOnline
              ? "Consultation booked!"
              : "Saved offline. Will book when connected."}
          </p>
          {isOnline && (
        ...
        <div className="space-y-4">
          <Button
            ...
            variant="primary"
            fullWidth
            className="h-[60px] text-lg rounded-full  shadow-primary/30"
          >
