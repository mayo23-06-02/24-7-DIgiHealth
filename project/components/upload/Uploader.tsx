  ...
  const onDrop = useCallback(
    async (acceptedFiles: File[]) => {
      ...
      try {
        ...
        const token = await fetchUploadToken().catch(
          () => "mock_token_for_demo",
        );
    ...
    <div className="space-y-4">
      ...
      >
        ...
        {preview ? (
          ...
          <>
            <div
              className={`text-5xl mb-6 transform group-hover:scale-110 transition-transform duration-500 ${isUploading ? "animate-pulse" : ""}`}
            >
              📸
            </div>
            ...
            <p className="text-xs text-slate-300 mt-2 font-medium">
              Supporting JPG, PNG, WEBP (Max 5MB)
            </p>
          </>
        )}
...
export const PDFUploader: React.FC<UploaderProps> = ({
  onUploadComplete,
  label,
  required,
}) => {
  ...
  return (
    <div className="space-y-4">
      ...
      <div
        {...getRootProps()}
        className={`w-full py-10 rounded-2xl border-2 border-dashed flex flex-col items-center justify-center bg-slate-50 transition-all ${isDragActive ? "border-primary" : "border-slate-200"}`}
      >
        <input {...getInputProps()} />
        {fileName ? (
