...
export default function DashboardHeader({ 
  ...
  return (
    <>
      <header
        className="sticky top-0 bg-white/95 backdrop-blur-md px-5 py-[7px] flex items-center justify-between z-[90] border-b border-slate-100 transition-all duration-500"
      >
      {/* Mobile Toggle & Branding */}
      <div className="flex items-center gap-4 lg:hidden mr-4">
         <button onClick={onMenuClick} className="w-10 h-10 bg-slate-50 rounded-lg flex items-center justify-center text-slate-500 hover:text-primary transition-colors">
      ...
      <div className="hidden sm:flex flex-1 flex flex-col gap-1 pr-10">
        <div className="flex items-center gap-3">
          <span className="text-[11px] font- text-slate-400 ">
            {formattedDate}
          </span>
      ...
      <div className="flex-[2] max-w-[600px] px-10">
        <div
          className="bg-slate-100 border border-slate-100 rounded-lg px-5 py-4 flex items-center gap-6 group focus-within:bg-white focus-within:border-primary/30 transition-all duration-500"
        >
      ...
      <div className="flex-1 flex items-center justify-end gap-6 pl-10">
          <div className="relative">
            <div 
              ...
              className={`w-10 h-10 rounded-lg border border-slate-100 flex items-center justify-center cursor-pointer transition-all relative ${
                isNotificationOpen ? "bg-primary text-white" : "bg-white text-slate-400 hover:text-primary hover:bg-primary/5"
              }`}
            >
              <BiBell size={18} />
            ...
            {isNotificationOpen && (
              <div className="absolute top-[50px] right-0 w-[380px] bg-white border border-slate-200 rounded-lg p-6 animate-in zoom-in-95 slide-in-from-top-4 duration-300 z-50">
                <div className="flex items-center justify-between mb-6">
                  <h4 className="text-lg font-black text-slate-800 tracking-tight">Health Notifications</h4>
                ...
                <div className="space-y-4 max-h-[400px] overflow-y-auto custom-scrollbar pr-2">
                  {[
                    { id: '1', title: "Medication Reminder", desc: "Time for your 500mg Amoxicillin dose. Please confirm once taken.", time: "2m ago", icon: <BiCapsule />, color: "text-supportive-teal bg-teal-50", type: "medication" },
                    { id: '2', title: "Lab Result Ready", desc: "Your Full Blood Count results are now available for review from Clinpath.", time: "1h ago", icon: <BiPulse />, color: "text-purple-500 bg-purple-50", type: "lab" },
                    { id: '3', title: "Appointment Accepted", desc: "Your upcoming checkup with Dr. Nkosi has been scheduled for tomorrow at 10:30 AM.", time: "3h ago", icon: <BiCheckCircle />, color: "text-emerald-500 bg-emerald-50", type: "appointment" }
                  ].map((notifItem, i) => (
                    <div 
                      key={notifItem.id} 
                      onClick={() => {
                    ...
                    >
                      <div className={`w-12 h-12 rounded-lg flex items-center justify-center shrink-0 ${notifItem.color}`}>
                        {notifItem.icon}
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="flex justify-between items-center mb-1">
                          <p className="text-sm font-bold text-slate-800 truncate">{notifItem.title}</p>
                          <span className="text-[10px] text-slate-400 font-bold whitespace-nowrap ml-2">{notifItem.time}</span>
                        </div>
                        <p className="text-xs text-slate-500 leading-relaxed truncate">{notifItem.desc}</p>
                      </div>
                    </div>
                  ))}
                </div>
                ...
                <button 
                  ...
                  className="w-full mt-6 bg-slate-100 hover:bg-primary hover:text-white py-3 rounded-lg text-[10px] font-black text-slate-500 uppercase tracking-widest transition-all active:scale-95"
                >
                  Enter Action Center
                </button>
              </div>
        ...
        <div className="group flex items-center gap-2 cursor-pointer hover:bg-slate-50 p-2 pr-6 rounded-lg transition-all">
          ...
          <div className="hidden lg:flex flex-col">
            ...
            <span className="text-[10px] text-primary">Premium Patient</span>
          </div>
        </div>
      </header>
        ...
        >
          <div className="pt-2">
            <div className="flex items-start gap-4 mb-6">
               <div className={`w-16 h-16 rounded-2xl flex items-center justify-center shrink-0 text-2xl ${selectedNotification.color}`}>
                 {selectedNotification.icon}
               </div>
               <div>
                  <h3 className="text-lg font-black text-slate-800 leading-tight mb-2">{selectedNotification.title}</h3>
                  <span className="text-[10px] font-bold uppercase tracking-widest text-slate-400 bg-slate-100 px-3 py-1 rounded-full">{selectedNotification.time}</span>
               </div>
            </div>
            
            <div className="bg-slate-50 p-5 rounded-2xl border border-slate-100 mb-8">
               <p className="text-sm text-slate-600 font-medium leading-relaxed">{selectedNotification.desc}</p>
            </div>
            ...
            <div className="flex flex-col gap-3">
               ...
               {selectedNotification.type === 'medication' && (
                 <button className="w-full py-3.5 rounded-xl bg-primary text-white hover:bg-primary/90 font-bold text-xs flex items-center justify-center gap-2 transition-all shadow-lg shadow-primary/20">
