...
import { BiX, BiUpload, BiCloudUpload, BiFile, BiLoaderAlt } from 'react-icons/bi';
...
export default function AttachRecordModal({ isOpen, onClose, conversationId }: AttachRecordModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      <div className="absolute inset-0 bg-slate-900/60 backdrop-blur-sm" onClick={isUploading ? undefined : onClose} />
      <div className="relative w-full max-w-lg bg-white rounded-3xl p-6 shadow-2xl animate-in zoom-in-95 duration-200">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-xs font-bold text-slate-500 uppercase tracking-wide mb-1.5 px-1">Record Type</label>
            <select 
              value={type} 
          ...
          <div>
            <label className="block text-xs font-bold text-slate-500 uppercase tracking-wide mb-1.5 px-1">Title</label>
            <input 
              type="text" 
              value={title}
          ...
          <div>
            <label className="block text-xs font-bold text-slate-500 uppercase tracking-wide mb-1.5 px-1">Notes / Description (Optional)</label>
            <textarea 
              value={description}
          ...
          <div>
            <label className="block text-xs font-bold text-slate-500 uppercase tracking-wide mb-1.5 px-1">File Attachment</label>
            <div 
            ...
            >
              {file ? (
                ...
                <>
                  ...
                  <p className="text-sm font-bold text-slate-600 mb-1">Click or drag & drop to upload</p>
                  <p className="text-[10px] text-slate-400 font-medium">Supports PDF, JPG, PNG, DICOM (Max 10MB)</p>
                </>
              )}
            </div>
            <input type="file" ref={fileInputRef} onChange={handleFileChange} className="hidden" accept=".pdf,.jpg,.jpeg,.png,.dcm" />
          </div>
          ...
          <div className="pt-4 flex gap-3">
            ...
            <button 
              ...
              className="flex-1 py-3.5 rounded-xl bg-primary text-white font-black uppercase tracking-widest text-xs flex items-center justify-center gap-2 hover:bg-primary/90 transition-all shadow-lg shadow-primary/20 disabled:opacity-50 disabled:shadow-none"
            >
              {isUploading ? <><BiLoaderAlt className="animate-spin" size={16} /> Uploading</> : 'Attach Record'}
            </button>
          </div>
        </form>
      </div>
