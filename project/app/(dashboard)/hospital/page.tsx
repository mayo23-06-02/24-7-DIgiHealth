...
import {
  BiGroup, BiDollarCircle, BiStar, BiCheckShield, BiLoaderAlt,
  BiUserPlus, BiBarChartAlt2, BiCalendar, BiChevronRight, BiTrendingUp,
} from "react-icons/bi";
...
const MOCK_STATS: DashboardStats = {
  totalStaff: 48, staffOnDuty: 31, revenueMonth: 284500, pendingPayouts: 12300,
  avgRating: 4.6, totalReviews: 214, slaCompliance: 88, avgResponseMin: 11.4,
...
const MOCK_REVIEWS: Review[] = [
  { _id: "1", patientName: "Thabo Nkosi", department: "Cardiology", rating: 5, comment: "Exceptional care from the entire team. Fast, professional, and genuinely compassionate.", createdAt: "2026-04-28T09:14:00Z" },
  { _id: "2", patientName: "Lerato Dlamini", department: "General", rating: 4, comment: "Very good experience overall. The teleconsultation was seamless and the doctor was thorough.", createdAt: "2026-04-25T14:30:00Z" },
  { _id: "3", patientName: "James van der Berg", department: "Neurology", rating: 5, comment: "Outstanding service. Appointment booked in minutes, doctor was on time and very knowledgeable.", createdAt: "2026-04-22T11:00:00Z" },
];
...
function KPI({ icon, label, value, sub, color }: { icon: React.ReactNode; label: string; value: string; sub: string; color: string }) {
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-100 p-6 flex flex-col gap-4 hover:shadow-md hover:border-slate-200 transition-all duration-200">
      ...
      <div>
        <p className="text-2xl font-bold text-slate-900 font-grotesk leading-none">{value}</p>
    ...
    const load = async () => {
      try {
        ...
        setStats({
          ...
          consultationsToday: kpi.consultationsToday ?? MOCK_STATS.consultationsToday,
        });
        ...
        setMonthly(
          perfJson?.data?.consultationVolume?.length
            ? perfJson.data.consultationVolume.map((m: any, i: number) => ({
                ...
                satisfaction: perfJson.data.satisfactionTrend?.[i]?.score ?? 4.5,
              }))
            : MOCK_MONTHLY
        );
  ...
  return (
    <div className="w-full pb-16 flex flex-col gap-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 font-grotesk">Facility Overview</h1>
          <p className="text-sm text-slate-500 mt-1">Live metrics and operational snapshot for today</p>
        </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-5 gap-6">
        ...
        <div className="xl:col-span-3 bg-white rounded-2xl border border-slate-100 p-6">
          <div className="flex items-center justify-between mb-6">
            <div>
              <h2 className="text-sm font-bold text-slate-800 font-grotesk">Monthly Consultations</h2>
              <p className="text-xs text-slate-400 mt-0.5">Last 6 months</p>
            </div>
            <span className="text-xs font-bold text-slate-400 border border-slate-100 px-3 py-1.5 rounded-lg">
              Today: {s.consultationsToday}
            </span>
          </div>
          <ResponsiveContainer width="100%" height={220}>
            <BarChart data={monthly} barSize={28} margin={{ top: 4, right: 8, left: -12, bottom: 0 }}>
              ...
              <Tooltip
                ...
                cursor={{ fill: "#f8fafc" }}
              />
              <Bar dataKey="consultations" name="Consultations" fill="#0052CC" radius={[6,6,0,0]}/>
            </BarChart>
          </ResponsiveContainer>
        </div>
        ...
        <div className="xl:col-span-2 bg-white rounded-2xl border border-slate-100 p-6">
          <div className="mb-6">
            <h2 className="text-sm font-bold text-slate-800 font-grotesk">Revenue & Satisfaction</h2>
            <p className="text-xs text-slate-400 mt-0.5">Dual trend overview</p>
          </div>
          <ResponsiveContainer width="100%" height={220}>
            <LineChart data={monthly} margin={{ top: 4, right: 8, left: -20, bottom: 0 }}>
              <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#f1f5f9"/>
              <XAxis dataKey="month" tick={{ fontSize: 11, fill: "#94a3b8", fontWeight: 600 }} axisLine={false} tickLine={false}/>
              <YAxis yAxisId="left" tick={{ fontSize: 10, fill: "#94a3b8" }} axisLine={false} tickLine={false} tickFormatter={(v) => `R${(v/1000).toFixed(0)}k`}/>
              <YAxis yAxisId="right" orientation="right" domain={[3.5,5]} tick={{ fontSize: 10, fill: "#94a3b8" }} axisLine={false} tickLine={false} tickFormatter={(v) => `${v}★`}/>
              ...
              <Line yAxisId="left" type="monotone" dataKey="revenue" name="Revenue (R)" stroke="#0052CC" strokeWidth={3} dot={{ r: 4, fill: "#fff", strokeWidth: 2 }} activeDot={{ r: 6 }}/>
              <Line yAxisId="right" type="monotone" dataKey="satisfaction" name="Rating (★)" stroke="#f59e0b" strokeWidth={3} dot={{ r: 4, fill: "#fff", strokeWidth: 2 }} activeDot={{ r: 6 }}/>
            </LineChart>
          </ResponsiveContainer>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div className="xl:col-span-2 bg-white rounded-2xl border border-slate-100 p-6 flex flex-col gap-5">
          <div className="flex items-center justify-between">
            <h2 className="text-sm font-bold text-slate-800 font-grotesk">Recent Patient Reviews</h2>
            <Link href="/hospital_admin/reviews" className="text-xs font-bold text-[#0052CC] hover:underline flex items-center gap-1">
          ...
          <div className="space-y-4">
            {reviews.map(r => (
              <div key={r._id} className="flex items-start gap-4 p-4 rounded-xl border border-slate-50 bg-slate-50/60 hover:bg-slate-100/40 transition-colors">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex flex-wrap items-center gap-2 mb-1">
                    <span className="text-sm font-bold text-slate-800">{r.patientName}</span>
                    <span className="text-[10px] font-bold text-slate-400 bg-slate-200 px-2 py-0.5 rounded-full">{r.department}</span>
                  </div>
                  <Stars rating={r.rating}/>
                  <p className="text-xs text-slate-500 mt-1.5 leading-relaxed line-clamp-2">{r.comment}</p>
                </div>
        ...
        <div className="flex flex-col gap-5">
          ...
          <div className="bg-white rounded-2xl border border-slate-100 p-6 flex flex-col gap-5">
            <h2 className="text-sm font-bold text-slate-800 font-grotesk">SLA Status</h2>

            <div>
              <div className="flex justify-between mb-2">
                <span className="text-xs font-bold text-slate-500">Response Time Target (≤15 min)</span>
                <span className={`text-xs font-bold ${s.avgResponseMin <= 15 ? "text-emerald-600" : "text-rose-500"}`}>
              ...
              <div className="w-full h-2.5 bg-slate-100 rounded-full overflow-hidden">
                <div
                  ...
                  style={{ width: `${Math.min((s.avgResponseMin / 15) * 100, 100)}%` }}
                />
              </div>
            </div>
            ...
            <div>
              <div className="flex justify-between mb-2">
                ...
                <span className={`text-xs font-bold ${slaOk ? "text-emerald-600" : "text-rose-500"}`}>{s.slaCompliance}%</span>
              </div>
              <div className="w-full h-2.5 bg-slate-100 rounded-full overflow-hidden">
                <div
              ...
              <div className="flex items-center gap-2 mt-3">
                {slaOk
                  ? <><div className="w-5 h-5 rounded-full bg-emerald-100 text-emerald-600 flex items-center justify-center text-xs">✓</div><span className="text-xs font-bold text-emerald-600">All SLA targets met</span></>
                  : <><div className="w-5 h-5 rounded-full bg-rose-100 text-rose-500 flex items-center justify-center text-xs">✗</div><span className="text-xs font-bold text-rose-500">Below compliance threshold</span></>
                }
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-slate-100 p-6 flex flex-col gap-3">
            <h2 className="text-sm font-bold text-slate-800 font-grotesk">Quick Actions</h2>
            {[
              { href: "/hospital_admin/staff",    icon: <BiUserPlus size={18}/>,      label: "Manage Staff",        sub: "Directory & scheduling" },
              { href: "/hospital_admin/reports",  icon: <BiBarChartAlt2 size={18}/>,  label: "Performance Reports", sub: "CSV export & analytics" },
              { href: "/hospital_admin/sla",      icon: <BiCheckShield size={18}/>,   label: "SLA Monitor",         sub: "Targets & compliance" },
              { href: "/hospital_admin/reviews",  icon: <BiStar size={18}/>,          label: "Patient Reviews",     sub: "Ratings & feedback" },
            ].map(a => (
              <Link key={a.href} href={a.href}>
                <div className="flex items-center gap-3 p-3 rounded-xl hover:bg-slate-50 border border-transparent hover:border-slate-100 transition-all group cursor-pointer">
                  ...
                  <BiChevronRight size={16} className="text-slate-300 group-hover:text-[#0052CC] transition-colors"/>
                </div>
              </Link>
            ))}
          </div>
