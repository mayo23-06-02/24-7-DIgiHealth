...
import {
  BiSearch, BiLoaderAlt, BiDownload, BiMessageDetail,
  BiPhone, BiCalendar, BiFilter
} from 'react-icons/bi';
...
export default function PractitionerPatientsPage() {
  ...
  const sorted = [...patients].sort((a, b) => {
    ...
    if (sortField === 'lastVisit') { aVal = aVal ? new Date(aVal).getTime() : 0; bVal = bVal ? new Date(bVal).getTime() : 0; }
    if (sortField === 'riskScore') { aVal = a.riskScore || 0; bVal = b.riskScore || 0; }
  ...
  const exportCSV = () => {
    const headers = ['Name', 'Age', 'Gender', 'Blood Type', 'Last Visit', 'Risk Score', 'Risk Level'];
    ...
    <div className="w-full pb-10 flex flex-col gap-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500">{patients.length} patients under your care</p>
        </div>
        <button onClick={exportCSV} className="flex items-center gap-2 px-4 py-2 border border-slate-200 rounded-lg text-sm font-bold text-slate-600 hover:bg-slate-50 transition-colors w-fit">
      ...
      <div className="flex flex-wrap gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <select value={riskFilter} onChange={e => setRiskFilter(e.target.value)} className="border border-slate-200 rounded-lg px-3 py-2 text-sm bg-white">
      ...
      <Card className="flex flex-col p-0 overflow-hidden">
            ...
            <table className="w-full text-left border-collapse min-w-[800px]">
              <thead>
                <tr className="bg-slate-50 border-b border-slate-100">
                  ...
                  <th className="py-3 px-5 text-xs font-bold text-slate-500 uppercase tracking-wider">Conditions</th>
                  <SortTh field="lastVisit" label="Last Visit" />
                  <th className="py-3 px-5 text-xs font-bold text-slate-500 uppercase tracking-wider">Next Appt</th>
              ...
              <tbody className="divide-y divide-slate-50 bg-white">
                    ...
                    <td className="py-4 px-5">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-[10px] text-slate-400 capitalize">{p.gender}</p>
                        </div>
                      </div>
                    </td>
                    <td className="py-4 px-5 text-sm text-slate-600">{p.age || '—'}</td>
                    <td className="py-4 px-5">
                      <div className="flex flex-wrap gap-1 max-w-[200px]">
                        {(p.medicalHistory || []).slice(0, 2).map((h: string, idx: number) => (
                          <span key={idx} className="text-[10px] px-2 py-0.5 bg-blue-50 text-blue-700 border border-blue-100 rounded-full font-medium">{h}</span>
                        ))}
                        {(p.medicalHistory || []).length > 2 && (
                          <span className="text-[10px] px-2 py-0.5 bg-slate-100 text-slate-500 rounded-full font-medium">+{p.medicalHistory.length - 2}</span>
                        )}
                      </div>
                    </td>
                    <td className="py-4 px-5 text-xs text-slate-500">
                      {p.lastVisit ? new Date(p.lastVisit).toLocaleDateString('en-ZA', { day: 'numeric', month: 'short', year: 'numeric' }) : '—'}
                    </td>
                    <td className="py-4 px-5 text-xs text-slate-500">
                      {p.nextAppointment ? new Date(p.nextAppointment).toLocaleDateString('en-ZA', { day: 'numeric', month: 'short' }) : '—'}
                    </td>
                    <td className="py-4 px-5">
                      <div className="flex items-center gap-2">
                        <RiskScoreCard score={p.riskScore || 0} color={p.riskColor || 'green'} size="sm" showRing={false} />
                        <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full border ${RISK_STYLES[p.riskColor] || RISK_STYLES.green}`}>
                          {RISK_LABELS[p.riskColor as keyof typeof RISK_LABELS] || 'Low'}
                        </span>
                      </div>
                    </td>
                    <td className="py-4 px-5 text-right">
                      <div className="flex justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        <Link
                          ...
                          className="px-3 py-1.5 text-[10px] font-bold bg-primary text-white rounded-lg hover:bg-primary/90 transition-colors"
                        >
                          View
                        </Link>
                ...
                {paginated.length === 0 && (
                  <tr><td colSpan={7} className="py-12 text-center text-slate-400">No patients found.</td></tr>
                )}
              </tbody>
            </table>
          </div>
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-between px-5 py-3 border-t border-slate-100 bg-slate-50">
            <span className="text-xs text-slate-500">{patients.length} patients total</span>
