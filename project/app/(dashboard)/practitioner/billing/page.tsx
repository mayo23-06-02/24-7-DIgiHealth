...
import { BiDollarCircle, BiTime, BiLoaderAlt, BiDownload, BiCalendar, BiFilter } from 'react-icons/bi';
...
export default function PractitionerBillingPage() {
  ...
  const [summary, setSummary] = useState({ totalEarned: 0, pendingPayouts: 0, lastPayoutAmount: 0, lastPayoutDate: null });
  ...
  const fetchBilling = async () => {
    ...
    try {
      ...
      if (json.success) { setTransactions(json.data); setSummary(json.summary); }
    } catch (e) { console.error(e); }
    finally { setLoading(false); }
  };

  useEffect(() => { fetchBilling(); }, [filterStatus, dateFrom, dateTo]);

  const filtered = transactions.filter(t => !search || (t.patientName || '').toLowerCase().includes(search.toLowerCase()));
  ...
  return (
    <div className="w-full pb-10 flex flex-col gap-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-800">Billing & Earnings</h1>
          <p className="text-sm text-slate-500">Track your consultation fees and payouts</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <KPICard label="Total Earned (Month)" value={`R ${summary.totalEarned.toLocaleString()}`} icon={<BiDollarCircle size={24} />} color="primary" />
        <KPICard label="Pending Payouts" value={`R ${summary.pendingPayouts.toLocaleString()}`} icon={<BiTime size={24} />} color="slate" />
        <KPICard label="Last Payout" value={`R ${summary.lastPayoutAmount.toLocaleString()}`} icon={<BiDollarCircle size={24} />} color="emerald"
          description={summary.lastPayoutDate ? new Date(summary.lastPayoutDate).toLocaleDateString('en-ZA') : 'No payouts yet'}
        />
      </div>
            ...
            <table className="w-full text-left border-collapse min-w-[650px]">
              <thead>
                <tr className="bg-white border-b border-slate-100">
                  ...
                  <th className="py-3 px-5 text-xs font-bold text-slate-500 uppercase tracking-wider">Amount</th>
                  ...
                  <tr key={i} className="hover:bg-slate-50/50 transition-colors">
                    <td className="py-4 px-5">
                      <div className="flex items-center gap-3">
                        ...
                        <p className="text-sm font-bold text-slate-700">{t.patientName || 'Unknown'}</p>
                      </div>
                    </td>
                    <td className="py-4 px-5 text-xs capitalize text-slate-600">{t.type || '—'}</td>
                    <td className="py-4 px-5 text-xs text-slate-500">{t.date ? new Date(t.date).toLocaleDateString('en-ZA') : '—'}</td>
                    <td className="py-4 px-5 text-sm font-bold text-slate-800">R {(t.amount || 0).toLocaleString()}</td>
                    <td className="py-4 px-5">
                      <span className={`text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-lg border ${STATUS_BADGE[t.status] || STATUS_BADGE.pending}`}>
                        {t.status || 'pending'}
                      </span>
                    </td>
                    <td className="py-4 px-5 text-right">
                      <button className="text-[10px] font-bold text-primary hover:underline">Invoice</button>
                    </td>
                  </tr>
                ))}
                {paginated.length === 0 && (
                  <tr><td colSpan={6} className="py-12 text-center text-slate-400">No transactions found.</td></tr>
                )}
              </tbody>
            </table>
          </div>
