...
import { BiSearch, BiLoaderAlt, BiNote, BiChevronDown, BiChevronUp, BiVideo, BiChat, BiClinic, BiDownload } from 'react-icons/bi';
...
export default function ConsultationsPage() {
  ...
  const hasSoap = (c: any) => c.soapNotes && Object.values(c.soapNotes).some(Boolean);
    ...
    <div className="w-full pb-10 flex flex-col gap-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500">History of completed consultations with SOAP notes and clinical data</p>
        </div>
      ...
      <Card className="flex flex-col p-0 overflow-hidden">
        {loading ? (
          ...
          <div className="divide-y divide-slate-100">
              ...
              <div key={i}>
                ...
                <div className="flex items-center gap-4 px-5 py-4 hover:bg-slate-50 transition-colors">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[10px] text-slate-400 line-clamp-1 mt-0.5">{c.reason || 'General consultation'}</p>
                  </div>
                  ...
                  <div className="text-xs text-slate-500 shrink-0 hidden md:block">
                    {c.scheduledStart ? new Date(c.scheduledStart).toLocaleDateString('en-ZA', { day: 'numeric', month: 'short', year: 'numeric' }) : '—'}
                  </div>
                  <div className="shrink-0">
                    <RiskScoreCard score={c.riskScore || 0} color={c.riskColor || 'green'} size="sm" showRing={false} />
                  </div>
                  <div className="flex items-center gap-2 shrink-0">
                    <button
                      onClick={() => setSoapModal({ isOpen: true, consultationId: c.consultationId, patientName: c.patientName })}
                    ...
                    >
                      {expanded === c.id ? <BiChevronUp size={16} /> : <BiChevronDown size={16} />}
                    </button>
                  </div>
                </div>
                  ...
                  <div className="px-5 pb-5 bg-slate-50 border-t border-slate-100">
                        ...
                        [
                          ...
                          c.soapNotes?.[key] ? (
                            <div key={key} className="bg-white rounded-xl p-4 border border-slate-100 shadow-sm">
                              <p className={`text-[10px] font-black uppercase tracking-wide mb-1.5 ${color}`}>{label}</p>
                              <p className="text-xs text-slate-600 leading-relaxed">{c.soapNotes[key]}</p>
                            </div>
                          ) : null
                        )
                      ) : (
                    ...
                    {c.aiRecommendations?.length > 0 && (
                      <div className="mt-3 bg-blue-50 border border-blue-100 rounded-xl p-4">
                        <p className="text-[10px] font-black text-primary uppercase tracking-wide mb-2">AI Recommendations</p>
                        <ul className="space-y-1">
                          {c.aiRecommendations.map((rec: string, ri: number) => (
                            <li key={ri} className="text-xs text-blue-700 flex items-start gap-1.5">
                              <span className="text-primary shrink-0 mt-0.5">→</span> {rec}
                            </li>
                          ))}
                        </ul>
                      </div>
            ...
            {paginated.length === 0 && !loading && (
              <div className="py-12 text-center text-slate-400">No past consultations found.</div>
            )}
          </div>
        )}
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-between px-5 py-3 border-t border-slate-100 bg-slate-50">
            <span className="text-xs text-slate-500">{consultations.length} records</span>
