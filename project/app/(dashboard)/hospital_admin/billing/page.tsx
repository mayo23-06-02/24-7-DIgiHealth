...
import { BiDollarCircle, BiTime, BiHealth, BiDownload, BiLoaderAlt, BiCalendar } from 'react-icons/bi';

export default function BillingPage() {
  const [transactions, setTransactions] = useState<any[]>([]);
  const [summary, setSummary] = useState({ totalRevenue: 0, pendingAmount: 0, medicalAidClaims: 0 });
  ...
  const sorted = [...transactions]
    .filter(t => {
      const name = `${t.patientId?.firstName} ${t.patientId?.lastName}`.toLowerCase();
  ...
  const statusBadge = (status: string) => {
    ...
    return `px-2 py-0.5 rounded-lg text-[10px] font-bold uppercase tracking-wider border ${styles[status] || styles.pending}`;
  };
    ...
    <div className="w-full pb-10 flex flex-col gap-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500">Revenue tracking and payment records</p>
        </div>
        <button onClick={exportCSV} className="flex items-center gap-2 px-4 py-2 rounded-lg border border-slate-200 text-sm font-bold text-slate-600 hover:bg-slate-50 transition-colors">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <KPICard label="Total Revenue" value={`R ${summary.totalRevenue.toLocaleString()}`} icon={<BiDollarCircle size={24} />} color="primary" />
        <KPICard label="Pending Payments" value={`R ${summary.pendingAmount.toLocaleString()}`} icon={<BiTime size={24} />} color="slate" />
        <KPICard label="Medical Aid Claims" value={`R ${summary.medicalAidClaims.toLocaleString()}`} icon={<BiHealth size={24} />} color="emerald" />
      </div>
      ...
      <Card className="flex flex-col p-0 overflow-hidden">
            ...
            <table className="w-full text-left border-collapse min-w-[700px]">
              <thead>
                <tr className="bg-white border-b border-slate-100">
                  ...
                  <th className="py-3 px-5 text-xs font-bold text-slate-500 uppercase tracking-wider cursor-pointer hover:text-primary" onClick={() => toggleSort('amount')}>
                  ...
                  <th className="py-3 px-5 text-xs font-bold text-slate-500 uppercase tracking-wider cursor-pointer hover:text-primary" onClick={() => toggleSort('timestamp')}>
                    Date {sortField === 'timestamp' ? (sortDir === 'asc' ? '↑' : '↓') : ''}
                  </th>
                </tr>
              </thead>
                ...
                {paginated.map((t, i) => (
                  <tr key={i} className="hover:bg-slate-50/50 transition-colors">
                    <td className="py-4 px-5 text-sm font-bold text-slate-700">
                      {t.patientId ? `${t.patientId.firstName} ${t.patientId.lastName}` : 'Unknown'}
                    </td>
                    <td className="py-4 px-5 text-xs text-slate-600 capitalize">{t.type?.replace('_', ' ')}</td>
                    <td className="py-4 px-5 text-sm font-bold text-slate-800">R {t.amount.toLocaleString()}</td>
                    <td className="py-4 px-5"><span className={statusBadge(t.status)}>{t.status}</span></td>
                    <td className="py-4 px-5 text-xs text-slate-500 capitalize">{t.paymentMethod?.replace('_', ' ')}</td>
                    <td className="py-4 px-5 text-xs text-slate-500">{new Date(t.timestamp).toLocaleDateString('en-ZA')}</td>
                  </tr>
                ))}
                {paginated.length === 0 && (
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-between px-5 py-3 border-t border-slate-100 bg-slate-50">
            <span className="text-xs text-slate-500">{sorted.length} transactions total</span>
